...
def center_sampling(batch_size, num_sample, noise_dim):
	noise = (torch.ones(batch_size, num_sample, noise_dim)*0.5)
	return noise
...
for epoch in range(num_epochs):
    for i, inputs in enumerate(loader):
        ...
        center_noises = center_sampling(batch_size, num_sample, noise_dim)
        
        ########## Decode ########
        z_c_concat = torch.cat([z, center_noises.view(-1, center_noises.size(2))], dim=1)
        ...
        center_recon_loss = mse(y_hat, y_unsqueeze)
        
#        G_optimizer.zero_grad()
#        center_recon_loss.backward(retain_graph=True)
#        G_optimizer.step()
        ...
        rand_noise = noise_sampling(batch_size, num_sample, noise_dim)
        
        ########## Decode ########
        z_c_concat = torch.cat([z, rand_noise.view(-1, rand_noise.size(2))], dim=1)
        y_hat = decoder(z_c_concat)        
        ...
        y_unsqueeze = (y[:,None,:]).expand(-1,num_sample,-1)
        y_unsqueeze = y_unsqueeze.contiguous().view(-1,y_unsqueeze.shape[2])        
        overall_recon_loss = mse(y_hat, y_unsqueeze)
        
        ########## G Loss ##########
        G_loss = nn.BCEWithLogitsLoss()(torch.squeeze(discriminator(y_hat_x_cat)), torch.ones(y_hat_x_cat.size(0)))
        
        ########## Div Loss ##########
        pair_div_loss = div.compute_pairwise_divergence(y_hat.view(batch_size,num_sample,-1), rand_noise)
        total_loss = G_loss + 0.3*pair_div_loss + center_recon_loss*5
        G_optimizer.zero_grad()
        total_loss.backward()
        G_optimizer.step()
    ...
    center_recon_loss_np = center_recon_loss.data.numpy()
    overall_recon_loss_np = overall_recon_loss.data.numpy()
    print(epoch, 'center_recon_loss: ', center_recon_loss_np, 'overall_recon_loss', overall_recon_loss_np, 'G_loss_np: ', G_loss_np, 'D_loss_np: ', D_loss_np, 'pair_div_loss_np: ', pair_div_loss_np)
